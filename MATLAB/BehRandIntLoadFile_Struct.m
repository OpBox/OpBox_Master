...
% if streaming time stamps are save in file (as in early file, when wasn't just summary) then remove them
% from first \r\nITI: 0 to \r\nts_
start_flag = sprintf('\r\nITI:');
idx_start_flag = strfind(file_char', start_flag);
if ~isempty(idx_start_flag)
    idx_start_flag = idx_start_flag(1);
    end_flag = sprintf('\r\nts_');
    idx_end_flag = strfind(file_char', end_flag);
    idx_end_flag = idx_end_flag(1);
    file_char = file_char([1:idx_start_flag-1, idx_end_flag:end]);
end
