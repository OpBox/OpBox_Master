...
% Crop events by _on, and then correspondingly crop _off events, so as to limit "bookends"
crop_data = data;
...
for i_field = 1:numel(field_names)
    ...
    if strncmp('ts_', temp_field_name, 3)
        tag = temp_field_name(end-2:end);
        if strcmp('_in', tag) || strcmp('_on', tag)
            ...
            % Crop corresponding _out or _off
            switch tag
                case '_in'
                    suffix = '_out';
                case '_on'
            ...
            field_corresp = [temp_field_name(1:end-3) suffix];
            crop_data.(field_corresp) = data.(field_corresp)(mask);
            % Crop corresponding trial names
        ...
        elseif strcmp('_out', temp_field_name(end-3:end)) || strcmp('_off', temp_field_name(end-3:end))
