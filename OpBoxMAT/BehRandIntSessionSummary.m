function [per_gostim_lick, per_nostim_lick, per_nogostim_lick, acc, rate_np, ts_np_in, med_npdur, med_lick] = BehRandIntSessionSummary(filename)
...
% MisMatch NP/Lick In & Out: likely due to when protocol stopped if multi-threaded or in middle or trial
if length(data.ts_np_in) > length(data.ts_np_out)
    fprintf('Too many NP Ins=%4d vs.=Outs %4d\n', length(data.ts_np_in), length(data.ts_np_out));
    data.ts_np_in = data.ts_np_in(1:length(data.ts_np_out));
end
if length(data.ts_lick_in) > length(data.ts_lick_out)
    fprintf('Too many Lick Ins=%4d vs.=Outs %4d\n', length(data.ts_lick_in), length(data.ts_lick_out));
    data.ts_lick_in = data.ts_lick_in(1:length(data.ts_lick_out));
end
